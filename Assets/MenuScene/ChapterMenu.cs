...
public class ChapterMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            LevelButton lb = Instantiate(levelButtonPrefab, Vector3.zero, Quaternion.identity, transform).GetComponent<LevelButton>();
            lb.transform.localPosition = newButtonPos;
            lb.SetLevel(level, Path.GetFileName(level));
            lb.chapterMenu = this;
